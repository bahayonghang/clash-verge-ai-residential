...
Kimi Code natively supports project custom agents under `.kimi-code/agents/` or `.agents/agents/`. This project still uses the built-in `coder` sub-agent plus this role skill/pull, and does not add project Trellis agents. The built-in `explore` sub-agent is read-only, so it cannot satisfy this agent's persistence contract. The main session dispatches the built-in `coder` sub-agent via the Agent tool with a prompt that:
...
4. Must not spawn `trellis-implement` or `trellis-check`

Kimi does not auto-inject SessionStart task context in this project's Trellis routing. Always pull context as required below.
...
If the user asks you to edit code, decline and tell the main session to dispatch the implement role (built-in `coder` plus `.kimi-code/skills/trellis-implement/SKILL.md`).
