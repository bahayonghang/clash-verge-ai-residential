...
- [x] AC3：不满足门禁的变更不能合入 `main`，满足门禁的授权维护流程仍可完成。
...
- 受保护的 PR #3 在矩阵检查运行期间保持 `mergeStateStatus=BLOCKED`。
- 同一 head `60d87226796818989cca39eea706416602e3ab38` 的五项检查全部成功后，PR #3 变为 `mergeStateStatus=CLEAN`。
- 该状态转换证明唯一管理员不能绕过门禁，同时满足门禁的维护 PR 可以继续完成。
