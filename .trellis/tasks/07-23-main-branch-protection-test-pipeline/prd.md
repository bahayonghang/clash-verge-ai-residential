...
- [ ] AC1：CI 子任务的本地完整门禁通过，并在 GitHub Actions 产生稳定、成功的必需检查。
- [ ] AC2：`main` 分支保护要求 AC1 的稳定检查通过，且阻止强制推送和删除。
- [ ] AC3：不满足门禁的变更不能合入 `main`，满足门禁的授权维护流程仍可完成。
- [ ] AC4：仓库文档准确说明本地自动化检查、GitHub CI 和人工集成验证的职责边界。
- [ ] AC5：GitHub API 回读结果与批准的保护策略一致，最终工作区不包含本地凭据或生成文件变更。
