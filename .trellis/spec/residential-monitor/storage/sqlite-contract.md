...
- `dimension_dict` 新增 kind `'chain'` 与 `'rule_group'`，不得覆写既有 `'rule'`。精确维度层物化 host / process / `rule_group` / chain / network；水位键 `hourly_dim_v2`。排名 LEFT JOIN `dimension_dict`，缺失 identity 为 `"__unknown__"`，`dimension_dict.value` 不得写入该哨兵。`filters.host` 为该哨兵时 raw 层匹配 `coalesce(s.host,'')=''`，维度层匹配 `h.dimension_id = 0`。`filters.process` 为该哨兵时 raw 层匹配进程缺失谓词，维度层匹配 `h.dimension_id = 0`。`filters.category` 为 `"__residential__"` 时 raw 层匹配 `primary_category_id is not null`，维度层匹配 `category_id != 0`；该字面量不得 intern。
