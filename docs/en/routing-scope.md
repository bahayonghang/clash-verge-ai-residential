...
| Extra small sites | `routing.extra` defaults to `false`. After it is on, site switches control injection; currently only `routing.extra_anyrouter` (default `true`) injects `DOMAIN-SUFFIX,anyrouter.top` and does not write `+.anyrouter.top` residential DNS. Closing the category or the site returns them to the original Profile |
...
- AnyRouter console: https://anyrouter.top/console
- AnyRouter API / Claude Code: https://anyrouter.top
- `www.anyrouter.top` may still return ESA 530 on a healthy edge; that is a third-party origin limit, not a success condition for this repository's routing fix
