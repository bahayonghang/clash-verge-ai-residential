...
| Extra small sites | `routing.extra` defaults to `true` and currently injects only `DOMAIN-SUFFIX,anyrouter.top`, covering the AnyRouter apex and subdomains; disabling returns them to the original Profile |
...
- AnyRouter product entry: https://anyrouter.top/
