...
| `routing.extra` | `ROUTE_EXTRA` | `true` | Routes the independently maintained small AI sites; currently includes `anyrouter.top` and its subdomains. | Independent of the existing large-product switches; disabling returns these sites to the original Profile. |
