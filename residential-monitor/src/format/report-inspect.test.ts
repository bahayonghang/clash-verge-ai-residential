...
describe("shouldSkipReportPaint", () => {
  it("skips live deltas while the reports route is open", () => {
    expect(shouldSkipReportPaint("reports", "connectionDelta", null, null)).toBe(true);
    expect(shouldSkipReportPaint("reports", "healthChanged", null, null)).toBe(true);
    expect(shouldSkipReportPaint("reports", "summaryChanged", null, null)).toBe(true);
    expect(shouldSkipReportPaint("reports", "alertChanged", null, null)).toBe(true);
  });

  it("paints bootstrap, live route, and error changes", () => {
    expect(shouldSkipReportPaint("reports", "bootstrap", null, null)).toBe(false);
    expect(shouldSkipReportPaint("live", "connectionDelta", null, null)).toBe(false);
    expect(shouldSkipReportPaint("overview", "healthChanged", null, null)).toBe(false);
    expect(shouldSkipReportPaint("reports", "connectionDelta", "缺口", null)).toBe(false);
    expect(shouldSkipReportPaint("reports", "connectionDelta", "缺口", "缺口")).toBe(true);
  });
});
