...
fn entry(key: &str) -> Option<(&'static str, &'static str)> {
    Some(match key {
        ...
        "dialog.backup_create.title" => ("创建备份", "Create backup"),
        "dialog.backup_create.file" => ("monitor.backup.sqlite3", "monitor.backup.sqlite3"),
        "dialog.backup_restore.title" => ("选择备份文件", "Select a backup file"),
        "dialog.backup_restore.file" => ("monitor.backup.sqlite3", "monitor.backup.sqlite3"),
        "dialog.diagnostics_export.title" => ("导出诊断信息", "Export diagnostics"),
        "dialog.diagnostics_export.file" => ("diag.json", "diag.json"),
        "dialog.filter.report" => ("报告文件", "Report files"),
        "dialog.filter.backup" => ("备份文件", "Backup files"),
        ...
        "error.font_list" => ("无法读取本机字体。", "Cannot read local fonts."),
