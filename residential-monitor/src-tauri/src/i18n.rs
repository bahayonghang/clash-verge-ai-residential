...
fn entry(key: &str) -> Option<(&'static str, &'static str)> {
    Some(match key {
        ...
        "export.html_rankings" => ("排名（与图表同一结果）", "Rankings (same result as the chart)"),
        ...
        "health.endpoint_missing.action" => (
            "检查地址或重新发现",
            "Check the address or discover again",
        ),
        ...
        "health.pid_mismatch.action" => (
            "重新发现后改用 TCP",
            "Discover again, then use TCP",
        ),
        ...
        "health.storage_failure.action" => (
            "打开恢复界面检查磁盘",
            "Open recovery and check the disk",
        ),
        ...
        "health.restore_failed.action" => ("当前可用库未覆盖", "The current usable database remains"),
        ...
        "session.pid_mismatch" => ("管道进程身份不匹配。", "Named pipe process identity mismatch."),
        ...
        "action.check_secret" => ("检查本机 secret 后重试", "Check the local secret, then retry"),
        "action.enable_tcp" => (
            "启用 TCP External Controller",
            "Enable the TCP external controller",
        ),
        ...
        "action.open_data_dir" => ("打开数据目录检查文件", "Open the data directory and check files"),
        ...
        "error.recovery_only" => ("当前处于恢复模式，不能保存设置。", "Recovery mode cannot save settings."),
        "error.recovery_only_probe" => ("恢复模式不能测试控制器。", "Recovery mode cannot probe the controller."),
        "error.recovery_only_targets" => ("恢复模式不能保存目标。", "Recovery mode cannot save targets."),
        "error.recovery_only_report" => ("恢复模式不能运行普通报告。", "Recovery mode cannot run a normal report."),
        "error.recovery_only_close" => ("恢复模式不能关闭连接。", "Recovery mode cannot close a connection."),
        ...
        "error.diagnostics" => ("诊断生成失败。采集未中断。", "Diagnostics failed. Collection did not stop."),
        ...
        "settings.non_loopback" => ("TCP 只接受本机回环地址。", "TCP accepts loopback addresses only."),
        ...
        "settings.check_controller" => ("检查控制器或凭据后重试", "Check the controller or credential, then retry"),
        "report.invalid_query" => ("查询参数无效。", "The query is not valid."),
        "report.capability_unsupported" => ("当前数据层不支持该查询。", "The current data tier does not support this query."),
        "report.cancelled" => ("查询已取消。", "The query was cancelled."),
        "report.deadline_exceeded" => ("查询超过时限。", "The query exceeded the deadline."),
        "report.token_expired" => ("报告快照已过期，请重新运行。", "The report snapshot expired. Run the report again."),
        "report.quota_exceeded" => ("报告快照配额已满。", "The report snapshot quota is full."),
        "report.storage_busy" => ("存储正忙。", "Storage is busy."),
        "report.insufficient_space" => ("磁盘空间不足，已停止。", "Disk space is insufficient. Stopped."),
        "report.failed" => ("存储失败。", "Storage failed."),
        "report.action.invalid_query" => ("检查时间范围、维度和分页", "Check the time range, dimension, and page"),
        "report.action.capability_unsupported" => ("缩小范围或改用支持的维度", "Narrow the range or use a supported dimension"),
        ...
        "report.action.storage_busy" => ("等待写入完成后再试", "Wait for writes to finish, then retry"),
        "report.action.insufficient_space" => ("清理磁盘后重试", "Free disk space, then retry"),
        "report.action.failed" => ("打开数据管理检查磁盘", "Open data management and check the disk"),
        _ => return None,
    })
}
